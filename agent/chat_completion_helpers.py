...
def _update_fallback_context_compressor(agent, context_length: int | None = None) -> None:
    ...
    respecting the explicit model.context_length config override. The window resolved before
    activation is reused when supplied, so the swap does not probe provider metadata twice."""
    compressor = getattr(agent, "context_compressor", None)
    if not compressor:
        return
    fb_context_length = context_length
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None, reset_at=None) -> bool:
    ...
    while True:
        ...
        fb_client = None
        fb_context_length: int | None = None
        fb_config_context_length: int | None = None
        try:
            ...
            # Resolve and validate the candidate context window before mutating any live
            # runtime state: a constrained profile may lower the default 64K floor, but a
            # fallback that cannot hold the tool loop must never be activated.
            if fb_provider != "moa":
                ...
                fb_config_context_length = fb.get("context_length")
                if isinstance(fb_config_context_length, bool):
                    fb_config_context_length = None
                elif fb_config_context_length is not None:
                    try:
                        fb_config_context_length = int(fb_config_context_length)
                    except (TypeError, ValueError):
                        fb_config_context_length = None
                    if fb_config_context_length is not None and fb_config_context_length <= 0:
                        fb_config_context_length = None
            ...
            # Never inherit the primary model's pin across providers: apply the fallback
            # entry's own context_length (None clears it) so its window is resolved fresh.
            # See #22387.
            agent._config_context_length = fb_config_context_length
            ...
            if fb_provider == "moa":
                ...
                if fb_api_mode == "anthropic_messages" and getattr(agent, "client", None) is None:
                    # The router's preliminary OpenAI-shaped client is superseded by the
                    # native Anthropic client and is no longer runtime-owned.
                    _close_candidate_clients(runtime_snapshot, fb_client)
            ...
            # Roll the runtime back before trying the next candidate: a half-applied fallback
            # must never leak into the chain or the caller's session state.
            ...
            _close_candidate_clients(
                runtime_snapshot, fb_client, candidate_client, candidate_anthropic_client)
