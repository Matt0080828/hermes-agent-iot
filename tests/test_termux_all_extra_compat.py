...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"hermes-agent-iot[termux]==0.21.3.post1"' in text
    assert '"hermes-agent-iot[matrix]' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"hermes-agent-iot[voice]' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
